...
from apps.section.models import FeatureIcecatProductConnection, FeatureGroup
...
def feature_by_group(group, product):
    features = FeatureIcecatProductConnection.objects.filter(product=product, group=group)
    if features:
        return {'features': features, "group":group }
    else: return {}

register.inclusion_tag('features.html')(feature_by_group)
